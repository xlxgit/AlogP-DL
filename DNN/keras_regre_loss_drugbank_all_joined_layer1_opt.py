...
# load dataset
model_id = "drugbank_all_logP_joined_relu_layer1_opt"
dataframe1 = read_csv("../ecfp-drugbank-all.csv", delimiter = ',')
dataframe2 = read_csv("../maccskeys-drugbank-all.csv", delimiter = ',')
...
Y = np.loadtxt("../drugbank_all_3d_AlogP.value")
...
def wider_model():
    # create model
    model = Sequential()
    model.add(Dense(1000, input_dim=2215, kernel_initializer='normal', activation='relu',kernel_regularizer=l2(0.0003), W_constraint=maxnorm(1)))
    model.add(Dense(20, kernel_initializer='normal', activation='relu',kernel_regularizer=l2(0.0003), W_constraint=maxnorm(1)))
...
for i in range(20):
    ...
    estimators =  KerasRegressor(build_fn=wider_model, epochs=100, batch_size=120, verbose=0)
    history=estimators.fit(X_train, Y_train, epochs=100, batch_size=120, validation_split=0.2)
...
fig = plt.figure()
plt.plot(history.history['loss'],label='training loss',c="black")
