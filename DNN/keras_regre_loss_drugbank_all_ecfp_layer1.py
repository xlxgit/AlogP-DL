...
# load dataset
model_id = "drugbank_all_logP_ecfp_layer1_"
...
def wider_model():
    # create model
    model = Sequential()
    model.add(Dense(1000, input_dim=2048, kernel_initializer='normal', activation='relu', kernel_regularizer=l2(0.0003), W_constraint=maxnorm(1)))
    model.add(Dense(10, kernel_initializer='normal', activation='relu',kernel_regularizer=l2(0.0003), W_constraint=maxnorm(1)))
...
for i in range(20):
    ...
    estimators =  KerasRegressor(build_fn=wider_model, epochs=100, batch_size=100, verbose=0)
    history=estimators.fit(X_train, Y_train, epochs=100, batch_size=100, validation_split=0.2)
