...
# load dataset
model_id = "drugbank_all_logP_joined_layer1_LSTM_noopt"
...
def wider_model():
    # create model
    model = Sequential()
    model.add(LSTM(200, input_dim=2215,return_sequences=True,input_shape=(n_timesteps,data_dim) ))
    model.add(LSTM(100,  ))
...
model =  KerasRegressor(build_fn=wider_model, nb_epoch=100, batch_size=20,  verbose=0)
...
for i in range(20):
    ...
    estimators =  KerasRegressor(build_fn=wider_model,  batch_size=20, verbose=0)
    history=estimators.fit(X_train, Y_train,   validation_split=0.2)
