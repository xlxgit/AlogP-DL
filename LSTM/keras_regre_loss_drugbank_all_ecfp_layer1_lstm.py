...
# load dataset
model_id = "drugbank_all_logP_ecfp_layer1_LSTM"
...
def wider_model():
    # create model
    model = Sequential()
    model.add(LSTM(2048, input_dim=2048,return_sequences=True,input_shape=(n_timesteps,data_dim) ))
    model.add(LSTM(10,  ))
...
for i in range(20):
    ...
    estimators =  KerasRegressor(build_fn=wider_model, nb_epoch=100,  batch_size=100, verbose=0)
    history=estimators.fit(X_train, Y_train, nb_epoch=100,  batch_size=100,  validation_split=0.2)
