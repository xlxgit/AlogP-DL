...
# load dataset
model_id = "drugbank_all_logP_joined_layer1_LSTM_opt"
#dataframe = read_csv("ecfp-drugbank-all.csv", delimiter = ',')
#dataframe = read_csv("../maccskeys-drugbank-all.csv", delimiter = ',')
...
X = dataset[:,:]
Y = np.loadtxt("../drugbank_all_logP.AlogP.value")
#Y = dataset[:,13]
print (len(X),len(Y))
print(X[1],Y[1])
...
def wider_model():
    # create model
    model = Sequential()
    model.add(LSTM(2215, input_dim=2215,return_sequences=True,input_shape=(n_timesteps,data_dim) ))
    model.add(LSTM(20,  ))
...
model =  KerasRegressor(build_fn=wider_model, nb_epoch=100, batch_size=120,  verbose=0)
...
for i in range(20):
    ...
    estimators =  KerasRegressor(build_fn=wider_model,nb_epoch=100,   batch_size=120, verbose=0)
    history=estimators.fit(X_train, Y_train, nb_epoch=100,   batch_size=120,   validation_split=0.2)
